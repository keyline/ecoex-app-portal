...
class Recycler extends Model{
    ...
    public function addInventoryData($data){
       $query = "INSERT INTO `ecoex_inventory` VALUES(null,'".$data['inventory_type']."','".$data['storeId']."','".$data['category']."','".$data['subCategory']."','".$data['product']."','".$data['item']."',
            '".$data['year']."','".$data['month']."','".$data['qty']."','".$data['unit']."','".$data['attachment']."','".$data['document_required']."',0,0,now(),now()) ";
        //echo $query;die;
        $this->db->query($query);        
    ...
    public function getInventoryDataByID($id){
        $query = "SELECT * FROM ecoex_inventory WHERE `inventory_id` = '".$id."' ";
        $query=$this->db->query($query);
        return $query->getRow();
    }
    public function getInventoryDataByState($id){
        $query = "SELECT * FROM ecoex_inventory_by_state WHERE `inventory_id` = '".$id."' ";
        $query=$this->db->query($query);
        return $query->getResultArray();
    }
    ...
    public function addInventoryByStateData($data){
        ...
        foreach($data['stateData'] as $key=>$stateValue){
            ...
            $stateRate = $data['rate'][$key];
            if($stateValue != '' && $stateQty != '0'){
                
                $query = "SELECT * FROM ecoex_inventory_by_state WHERE `inventory_id` = '".$data['inventoryID']."' && `state_id` =  '".$stateValue."'";
                $query=$this->db->query($query);
    ...
    public function getInventoryList($id){
        $query = "SELECT * FROM ecoex_inventory WHERE `c_id` = '".$id."' ORDER BY `inventory_id` DESC";
        $query=$this->db->query($query);
        return $query->getResultArray();
    }
    
    public function getAllInventoryList(){
        $query = "SELECT * FROM ecoex_inventory_by_state ORDER BY `id` DESC";
        $query=$this->db->query($query);
        return $query->getResultArray();
    }
    
    public function getAllocateQtyInventoryState($id){
        $query = "SELECT SUM(`req_qty`) as allocateQty FROM ecoex_inventory_by_state WHERE `inventory_id` = '".$id."' ";
        $query=$this->db->query($query);
        return $query->getRow();
    }
    public function getStateAllocateDataByState($target){
        $query = "SELECT * FROM ecoex_inventory_by_state WHERE `inventory_id` = '".$target."' ";
        $query=$this->db->query($query);
        return $query->getResultArray();
    }
    public function editInventoryData($data){
       $query = "UPDATE `ecoex_inventory` SET `c_id` = '".$data['storeId']."',`inventory_type` = '".$data['inventory_type']."',`categoryId` = '".$data['category']."',`sucCatId` = '".$data['subCategory']."',`productId` = '".$data['product']."',`itemId` = '".$data['item']."',`year` = '".$data['year']."',`month` = '".$data['month']."',`qty` = '".$data['qty']."',`unit` = '".$data['unit']."',`document_required` = '".$data['document_required']."',`updatedAt` = now() WHERE `inventory_id` = '".$data['inventoryID']."' ";
        //echo $query;die;
        $this->db->query($query);
        
        if($data['attachment'] != ''){
           $query = "UPDATE `ecoex_inventory` SET `attachment` = '".$data['attachment']."' WHERE `inventory_id` = '".$data['inventoryID']."' ";
            $this->db->query($query);
        }
    ...
    public function getAllocateQtyDistrict($id,$stateId){
        $query = "SELECT SUM(`req_qty`) as allocateQty FROM ecoex_inventory_by_district WHERE `inventory_id` = '".$id."'  && `state_id` = '".$stateId."'";
        $query=$this->db->query($query);
        return $query->getRow();
    }
