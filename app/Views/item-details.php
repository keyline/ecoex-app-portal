                ...
                <h2>                 
                  ...
                  <sup> 
                    <?php if($itemDetails['listing_type'] == 'SELL'){?>
                        <div class="tag-text salebg badge badge-danger" style="font-size: 14px;background: #ff0000; "><?=$itemDetails['listing_type']?></div>
                    <?php }?>
                    <?php if($itemDetails['listing_type'] == 'BUY'){?>
                        <div class="tag-text badge badge-success" style="font-size: 14px;background: #7bc900;"><?=$itemDetails['listing_type']?></div>
                    <?php }?>
                  </sup>
                </h2>
                ...
                <p>Collection : <span><?=$itemDetails['month']?> <?=$itemDetails['year']?></span></p>
                  <form id="submitInquiry" method="POST" action="marketPlaceSubmitInquiry">
                    ...
                    <div class="form-group">
                      ...
                      <input type="text" class="form-control requiredCheckInquiry" id="qty" name="qty" style="max-width:200px;" data-check="Quantity" autocomplete="off" onkeypress="return isNumber(event)">
                      <span class="badge badge-danger" id="qty-err"></span>
                    </div>
                    <div class="form-group">
                      <label for="">Required Documents</label>
                      <div class="row">
                        <?php $document_required = json_decode($itemDetails['document_required']); ?>
                        <?php if($itemDetails['listing_type'] == 'SELL'){ if($documentLists){ foreach($documentLists as $documentList){?>
                        <div class="col-md-6">
                          ...
                          <label for="document_list<?=$documentList->id?>" style="font-weight: bold;"><?=$documentList->documentName?></label>
                        </div>
                        <?php } } } else {?>
                          <ul class="list-group">
                            <?php
                              if(count($document_required)>0){ for($d=0;$d<count($document_required);$d++){
                                $document = $common_model->find_data('ecoex_document_list', 'row', ['id' => $document_required[$d]]);
                            ?>
                              <li class="list-group-item"><i class="fa fa-check text-success"></i> <?=(($document)?$document->documentName:'')?></li>
                              <input type="hidden" name="document_list[]" value="<?=$document_required[$d]?>">
                            <?php } }?>
                          </ul>
                        <?php }?>
                      </div>                      
  ...
  <script type="text/javascript">
    $(document).ready(function(){
      ...
      $('#qty').on('blur', function(){
        ...
        if(qty<=0){
          $('#qty-err').show();
          $('#qty-err').text('Quantity Not Less Than Or Equal Zero !!!');
          $(this).val('');
        } else {
          if(qty>max_qty){
            $('#qty-err').show();
